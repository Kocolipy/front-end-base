...
There is **no authentication** in this baseline, so there is no `setup` project,
no `storageState`, and no `.env` — every spec starts from a clean context and
`fullyParallel` is on.

### Routing a new spec

`playwright.config.ts` has one project, `smoke`, and its `testMatch` names its
specs explicitly (`/smoke\.spec\.ts/`) rather than globbing `*.spec.ts`. **A
...
When auth arrives, the shape to copy is one project per auth need — a `setup`
project that signs in once and saves `storageState`, an `authed` project
depending on it, an `auth-flows` project with explicitly empty storageState for
specs that drive sign-in themselves, and `guest` for the rest. Per-test
`login()` under `fullyParallel` fires N concurrent logins that throttle and time
out; storageState collapses that to one.
